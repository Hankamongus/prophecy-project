    ...
    <div>
        ...
        <div class="flex-col flex w-full justify-center mt-24">
            <div class="flex w-full justify-center">
                <img src="https://cfl-mango.s3.amazonaws.com/uploads/downloads/proto-evangelium-transparent-bg.png"
        ...
        <div style="display: flex; width: 100%; justify-content: center;">
            <p class="font-serif text-red-800 text-sm sm:text-xl md:text-2xl border-red-800 border-b rounded-lg"><strong>{{
                text3 }}</strong></p>
        </div>
        <!--Speaker Profile-->
        ...
        <div class="w-full flex flex-col items-center justify-center py-4">
            <div class="max-w-screen-md text-white">
                ...
                <div
                    class="h-auto w-7/8 sm:w-auto z-6 p-16 bg-slate-700 rounded-lg backdrop-blur rounded-lg p-8 space-y-8 z-10 relative">
                    <!-- Shadow Block -->
                    <div class="absolute inset-0 h-full w-full translate-x-4 -translate-y-4 bg-slate-800 rounded-lg" />

                    <div
                        class="h-auto w-7/8 sm:w-auto p-8 relative translate-x-4 -translate-y-4 bg-slate-800 mb-8 backdrop-blur rounded-lg border-sky-800 p-8 space-y-8 z-10">
                        <div class="flex justify-between text-sm md:text-2xl mb-5">
                            ...
                            <div class="text-gray-400 sm:text-sm md:text-2xl mb-5 font-serif"><strong>Descriptions:</strong>
                            </div>
                            <div class="text-gray-400 text-sm md:text-2xl mb-5 font-serif"><strong>Fulfillments:</strong>
                            </div>
                        </div>
                        <div v-for="p in prophecies" @click="selectProphecy(p)" :key="p"
                            class="flex justify-between space-x-4 space-y-1 hover:bg-slate-600/30 rounded-lg border-gray-500 border-b text-center">
                            <div
                                class="py-4 text-sky-800 text-center dark:hover:text-red-600 dark:hover:font-normal dark:text-red-800 uppercase">
                                {{ p.Prophecy }}</div>
                            <div class="py-2 border-gray-500  capitalize font-bold font-serif ">{{ p.Descriptions }}</div>
                            <div class="py-4  text-sky-800 text-center dark:hover:text-red-600 dark:text-red-800 uppercase">
                                {{ p.Fulfillments }}</div>
                        </div>
                    </div>

                </div>
                <div v-if="selectedProphecy" @click="selectedProphecy = null"
                    class="fixed w-full h-screen flex items-center justify-center bg-black/50 backdrop-blur z-50 top-0 left-0">
